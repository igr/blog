...
description: >
  Kao da postoji nekakav zoološki vrt softverskog razvoja u kome se čuvaju životinje za koje smo čuli, ali ih retko posećujemo.
---
...
I to je, praktično, sve šta rade. Ideja je da potpuno enkapsuliraju _ponašanje_ - koje je ništa više nego mašina stanja (state machine). Kada je enkapsulacija ostvarena ne treba više brinuti o lokovanju, jer svaki aktor/proces radi sa svojom mašinom stanja koja je nezavisna od ostalih. Svaki aktor ima svoj FIFO red, poštansko sanduče za poruke koje se obrađuju jedna za drugom; te nije potrebna dodatna sinhronizacija poruka.
...
⚠️ Da ne bi opteretili pojam "stanje", hajde da koristimo termine "**ponašanje**" i "**podaci**" (ili "mašina stanja ponašanja" i "stanje podataka".) `Counter` ima samo jedno ponašanje (ono u kome broji), ali zato puno mogućih stanja podataka (izbrojanih vrednost.)
...
Smatram da je važno da razumemo šta je Alan zamislio; ne zbog toga da ustanovimo da li je u pravu, koliko do toga da prepoznamo kada se originalne ideje zamene nekakvim, ponekada i dijametralno suprotnim, polu-skuvanim stavovima.

Idemo dalje. Postoji nešto što me žulja kada razmišljam o Erlang/Akka aktorima u kontekstu OOP. Naime, programer je taj instancira aktore i ne postoji dalji formalizam oko toga šta oni predstavljaju niti dalji red kako se izvršavaju. Aktori su u tom svetlu samo još jedan način paralelizacije od nekoliko dostupnih. Niko ti ne brani da aktiviraš koliko god instanci istog aktora koji rade sa istom tabelom u bazi i da potencijalno dođeš do sličnih problema koje bi imali i u tradicionalnoj arhitekturi.
...
Moja razmišljanja o OOP zapravo nisu krenula od aktora. Krenula su od enkapsulacije stanja podataka i nastavila se kasnije na diskusije sa [Dejanom](https://www.linkedin.com/in/dejanmilicic/) (ugrabite njegova razmišljanja ukoliko dobijte priliku!)
...
Opisani Traktori oduzimaju deo funkcionalnosti baze koji se bavi paralelizmom i ostavlja deo koji se bavi podacima. To radimo ne zato što baze ne rade kako treba, već iz drugih razloga:

1. rešavanje paralelnog pristupa na bazi je suviše kasno,
2. ukoliko je paralelizam rešen pre, u aplikativnom sloju, otvara nam se ceo jedan novi horizont aplikativnih mogućnosti, a koje baze ne mogu da ponude.

## Ima li šta još?

Izneta ideja me proganja već neko vreme i morala da je da završi primerom. Trudio sam se da ne tražim Akka alternative, upravo da bih mogao da sam sažvaćem ideju do kraja. Kako se ispostavilo, nešto slično već postoji - reč je o potcenjenom Microsoftovom projektu "Orleans". On uvodi pojam Vrituelnih Aktora koji na sličan način postoje u memoriji i koji su kontrolisani od strane frejmvorka, a ne programera. Nisam stigao da se posvetim detaljima, ali sve što sam usput načuo o Orleansu liči na iznetu ideju gore.

Kada sistem sam kontroliše život (tr)aktora, ostavlja mogućnost za interesantne funkcije; kao što bi bilo raspoređivanje traktora koji češće komuniciraju zajedno na istu instancu, kako bi smanjili kašnjenje komunikacije mrežom.

Ako se udubimo u problem, na koju god stranu da pogledamo uočava se nekakav trend koji nagoveštava i naglašava vrednost postojanja _autonomnih programskih jedinica_ (aktora, traktora, virtualnih aktora, ćelija...).

## Za kraj

Deluje mi da ideja o traktorima i flotama može napredovati.
